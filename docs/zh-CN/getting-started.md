...
如果宿主没有提供匹配的当前会话收据，每个 Route Resource 都带有 `readState: read-required`：
...
只有在 Agent 已读取精确资源后才能记录收据。静态收据在 Digest 不变时继续有效；动态 View 收据还必须匹配当前 Route Revision。示例见 [`examples/dynamic-resource/resource-read-receipts.example.json`](../../examples/dynamic-resource/resource-read-receipts.example.json)。
