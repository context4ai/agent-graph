...
[Context](https://github.com/context4ai/context) uses Agent Graph beneath two thin Skill entries—[`init.md`](https://github.com/context4ai/context/blob/main/packages/context-cli/plugin/commands/init.md) at 225 words and [`continue.md`](https://github.com/context4ai/context/blob/main/packages/context-cli/plugin/commands/continue.md) at 619. These 844 words replace embedding 50 Markdown workflow documents in Skills: 25 Action contracts and 57 referenced resources stay separate, while Agents still receive, read, and prove consumption of the exact files selected by each Route. Re-entering a Skill with unchanged Facts selects the same Route; completed evidence advances or no-ops, and revision-bound host commands reject stale writes. This gives Agent work an explicit idempotency boundary across retries and new turns. Its debug recording makes every selected Route, loop, Gate, and outcome replayable.
